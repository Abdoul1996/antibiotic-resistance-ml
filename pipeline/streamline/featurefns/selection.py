    ...
    #     'Interesting' features determined by union of feature selection results (from different algorithms)
        if num_algorithms > 1:
            for i in range(self.n_splits):
                ...
                if max_features_to_keep is not None and len(union_list) > max_features_to_keep:
            ...
            for i in range(self.n_splits):
                ...
                informative_count = len(feature_list)
                informative_feature_counts.append(informative_count)
                uninformative_feature_counts.append(total_features - informative_count)
                # Apply further filtering if more than max features remains
                if max_features_to_keep is not None and len(feature_list) > max_features_to_keep:
                    # Create score list dictionary with indexes in union list
                    new_feature_list = []
                    k = 0
                    while len(new_feature_list) < max_features_to_keep:
