    ...
    def job(self):
            ...
            try:
                ...
                if self.training:
                    self.analysis_report.image(
                        self.experiment_path + '/' + self.datasets[m] + '/exploratory/initial/ClassCountsBarPlot.png', 68, 47, 45,
                        35)
                    # upper left hand coordinates (x,y), then image width then height (image fit to space)
                else:
                    self.analysis_report.image(
                        self.experiment_path + '/' + self.train_name + '/replication/' + self.datasets[
                            m] + '/exploratory/initial/ClassCountsBarPlot.png', 68, 47, 45, 35)
                    # upper left hand coordinates (x,y), then image width then height (image fit to space)
            except Exception:
                self.analysis_report.set_font('Times', 'B', 10)
                self.analysis_report.x = 70
                ...
                self.analysis_report.cell(45, 4, 'Class Balance (Processed)', 1, align="L")
                self.analysis_report.set_font('Times', '', 8)
                self.analysis_report.x = 72
                self.analysis_report.y = 60
                self.analysis_report.cell(40, 4, 'No Class Count Plot Available', 1, align="L")
                pass
        ...
        if self.training:
            ...
            for k in range(len(self.datasets)):
                ...
                # Insert Mutual Information feature importance plot
                try:
                    self.analysis_report.image(
                        self.experiment_path + '/' + self.datasets[
                    ...
                    self.analysis_report.set_xy(5, 50)
                    self.analysis_report.cell(100, 10, 'Mutual Information Plot Not Available', 1, align="C")
                
                # Insert MultiSURF feature importance plot
                try:
                    self.analysis_report.image(
                    ...
                    self.analysis_report.set_xy(105, 50)
                    self.analysis_report.cell(100, 10, 'MultiSURF Plot Not Available', 1, align="C")
                ...
                self.analysis_report.cell(0, 8,
                                          ...
                                          align="L")
                # Insert composite feature importance plot
                try:
                    self.analysis_report.image(
                        self.experiment_path + '/' + self.datasets[
                    ...
                    self.analysis_report.set_xy(1, 170)
                    self.analysis_report.cell(208, 10, 'Composite Feature Importance Plot Not Available', 1, align="C")
                self.footer()
