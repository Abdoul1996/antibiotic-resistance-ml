...
class Dataset:
    ...
    def feature_correlation(self, experiment_path, x_data=None, plot=True, show_plots=False, initial=''):
        ...
        # Guardrail: correlation matrices are O(n_features^2) in memory and will OOM for
        # high-dimensional datasets (e.g., SNP matrices with tens of thousands of features).
        # In those cases we skip correlation export rather than crashing the pipeline.
        MAX_FEATURES_FOR_CORR = 5000
        if x_data.shape[1] > MAX_FEATURES_FOR_CORR:
            logging.warning(
                "Skipping feature correlation computation for %s: %d features exceeds limit (%d).",
                self.name,
                x_data.shape[1],
                MAX_FEATURES_FOR_CORR,
            )
            # Write a small marker file so downstream report generation can handle the missing plot gracefully.
            os.makedirs(experiment_path + '/' + self.name + '/exploratory/' + initial, exist_ok=True)
            with open(
                experiment_path + '/' + self.name + '/exploratory/' + initial + 'FeatureCorrelations_SKIPPED.txt',
                'w',
            ) as f:
                f.write(
                    f"Skipped feature correlation: n_features={x_data.shape[1]} exceeds limit={MAX_FEATURES_FOR_CORR}\n"
                )
            return
